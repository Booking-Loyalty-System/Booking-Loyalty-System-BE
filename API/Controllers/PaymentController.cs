...
public class PaymentController : ControllerBase
{
    ...
    public async Task<IActionResult> PaymentCancel(
          ...
          [FromQuery] string code)
    {
        // Gọi Service xử lý hủy (bên trong hàm này sẽ chuyển trạng thái thành Failed và bắn SignalR "Cancelled")
        await _paymentService.ProcessPayOsReturnAsync(orderCode, status, code);

        // Redirect về giao diện Front-end
        return Redirect($"http://localhost:5173/staff/dashboard?paymentStatus=cancel&orderCode={orderCode}");
    }
